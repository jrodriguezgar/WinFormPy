        ...
        try:
            ...
            if pk:
                ...
                if success:
                    ...
                    print("❌ Delete failed: Backend returned False")
            else:
    ...
    class CrudDemoBackend(DataGridBackend):
        ...
        def create_record(self, record: Dict[str, Any]) -> Dict[str, Any]:
            ...
            if "price" in new_record:
                ...
                except Exception: new_record["price"] = 0.0
            if "stock" in new_record:
                try:
                    new_record["stock"] = int(new_record["stock"])
                except Exception: new_record["stock"] = 0
            ...
            for i, record in enumerate(self._data):
                if record.get("id") == primary_key_value:
                    ...
                    if "price" in changes:
                        ...
                        except Exception: changes["price"] = record.get("price", 0.0)
                    if "stock" in changes:
                        try:
                            changes["stock"] = int(changes["stock"])
                        except Exception: changes["stock"] = record.get("stock", 0)
