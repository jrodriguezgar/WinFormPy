...
class PDFBackend(DocumentBackend):
    ...
    def _create_error_page(self, message: str) -> Image.Image:
...
class WordBackend(DocumentBackend):
    ...
    def _ensure_docx(self):
        ...
        try:
            ...
            self.page_count = max(1, len(self.doc.paragraphs) // 20)  # Approximate
            self.current_page = 0
            self.document_loaded = True
            return True
        except ImportError as e:
            raise 
            self.doc = self.Document(file_path)
            self.file_path = file_path
            # For Word, treat each section or page break as a page
            # For simplicity, we'll treat the whole document as pages based on paragraphs
            self.page_count = max(1, len(self.doc.paragraphs) // 20)  # Approximate
    ...
    def render_page(self, page_number: int, width: int = None, height: int = None) -> Image.Image:
